...
// TÁC DỤNG: Quản lý ví, tạo địa chỉ bc1q SegWit, testnet, đa ví
// FIX LẦN 2: Bỏ isP2PKH, dùng upgradeToDeterministic trực tiếp
...
class WalletManager(private val context: Context) {
    ...
    fun start() {
        ...
        // SAU KHI VÍ CHẠY, ÉP DÙNG SEGWIT BC1Q
        try {
            val wallet = walletKit?.wallet()
            // Nếu ví chưa phải SegWit, nâng cấp lên P2WPKH (bc1q)
            // Lệnh này an toàn, nếu đã là bc1q thì không làm gì
            wallet?.upgradeToDeterministic(Script.ScriptType.P2WPKH, null)
        } catch (e: Exception) {
            // Bỏ qua lỗi nếu ví cũ không upgrade được
        }
    }
